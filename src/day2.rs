...
impl Round {
    fn points(&self) -> u32 {
        ...
        if self.you == Move::Rock && self.elf == Move::Scissors {
            return move_point + 6;
        }
        if self.you == Move::Paper && self.elf == Move::Rock {
            return move_point + 6;
        }
        if self.you == Move::Scissors && self.elf == Move::Paper {
            return move_point + 6;
        }
        move_point
    }
    fn points_round2(&self) -> u32 {
        if self.you == Move::Paper {
            return self.elf.value() + 3;
        }
        if self.you == Move::Rock {
            if self.elf == Move::Paper {
                return 1;
            }
            if self.elf == Move::Scissors {
                return 2;
            }
            if self.elf == Move::Rock {
                return 3;
            }
        }
